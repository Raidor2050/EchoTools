...
import { EASE_IN, EASE_OUT, fadeUp, stagger } from "@/lib/motion"
...
export function Hero() {
  ...
  return (
    <section className="relative overflow-hidden">
      <div
        aria-hidden
        className="pointer-events-none absolute inset-x-0 -top-40 z-0 h-[34rem] blur-2xl motion-reduce:hidden"
        style={{
          background:
            "radial-gradient(60% 55% at 50% 0%, oklch(var(--accent-l) var(--accent-c) var(--accent-h) / 0.24), transparent 70%)",
        }}
      />
      <motion.div
        variants={stagger}
        initial="hidden"
        animate="visible"
        className="relative z-10 mx-auto max-w-7xl px-4 pb-20 pt-20 text-center sm:px-6 sm:pt-28"
      >
        <AnimatePresence mode="wait">
          <motion.div
            ...
            exit={{ opacity: 0, y: -10, transition: { duration: 0.25, ease: EASE_IN } }}
        ...
        <motion.dl
          variants={fadeUp}
          className="mx-auto mt-14 grid max-w-2xl grid-cols-2 gap-px overflow-hidden rounded-xl border border-line-subtle bg-line-subtle sm:grid-cols-4"
        >
        ...
        <motion.div
          variants={fadeUp}
          className="mt-8 flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-xs text-faint"
        >
